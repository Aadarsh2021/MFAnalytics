...
import { detectRegime, getMissingAssetClasses, validateRegimeConstraints, getRegimeAllocationBands, shouldExitRegimeC } from '../../utils/regime/regimeDetector'
import { processMacroData } from '../../utils/regime/macroDataProcessor'
import { backtestRegimePortfolio, generateBacktestReport } from '../../utils/backtest/backtestEngine'
import { REGIMES } from '../../config/regimeConfig'
import { getLiveIndianData } from '../../utils/api/LiveIndianDataService' // Import Live Service
...
import SixPillarsDashboard from '../dashboard/SixPillarsDashboard'

import { supabase } from '../../utils/api/supabase'
import { fetchWithProxy, fetchBatch } from '../../utils/api/apiOptimized'
...
export default function Step4ARegimeViews({
    ...
    console.log('Step4ARegimeViews Render:', { macroDataLength: macroData?.length, isLiveUpdating })
    ...
    const fetchLiveUSData = async () => {
        ...
        try {
            ...
            if (!cacheErr && cache) {
                ...
                if (diffHours < 24) {
                    const yyyyMm = cache.date.substring(0, 7);
                    ...
                    if (!merged.find(d => d.date === yyyyMm)) {
            ...
            try {
                ...
                alert("US Data updated successfully!");
            } catch (e) {
            ...
            console.error('US Live Fetch failed details:', err);
            setIsLiveUpdating(false);
            alert(`Live US Update failed: ${err.message}. Please check console for details.`);
            // Removed automatic fallback to allow user choice
        } finally {
            setIsLiveUpdating(false);
        }
    };
    ...
    const fetchLiveIndiaData = async () => {
        ...
        try {
            ...
            try {
                ...
                alert("India Data updated successfully!");
            } catch (e) {
                console.error("Error syncing India data to Supabase", e);
            }
        } catch (err) {
            console.error('India Fetch failed details:', err);
            alert(`Live India Update failed: ${err.message}. Falling back to static data.`);
            // Removed automatic fallback to allow user choice
        } finally {
            setIsLiveUpdating(false);
        }
